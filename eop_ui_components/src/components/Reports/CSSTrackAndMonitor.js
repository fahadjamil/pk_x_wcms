...
export const CSSTrackAndMonitor = (props) => {
    const { lang } = props;
                ...
                >
                    <thead className="table-primary">
                        <tr>
                            ...
                            <td style={{ backgroundColor: '#006622', color: '#fff', }} scope="col">Category</td>
                            ...
                            <td style={{ backgroundColor: '#006622', color: '#fff', }} scope="col">Modified By</td>
                            <td style={{ backgroundColor: '#006622', color: '#fff', }} scope="col">Modified Time</td>
                        </tr>
                    </thead>
                    <tbody>
                        {data.applications &&
                            data.applications.map((item, index) => {
                                return (
                                    <tr key={item._id}>
                                        <td>
                                            {item.application_id.reference_number
                                                ? item.application_id.reference_number
                                                : ''}
                                        </td>
                                        <td>
                                        ...
                                        <td>
                                            {item.application_id.submit_time
                                                ? moment(item.application_id.submit_time).format(
                                                    'DD-MM-YYYY hh:mm A'
                                                )
                                                : ''}
                                        </td>
    ...
    const columns = [
        {
            name: '',
            selector: (row) => (row._id != undefined ? row._id.department_name : ''),
        ...
        <div style={{ display: loading ? 'none' : 'block' }}>
            {authorize ? (
                <div>
                    <form
                        onSubmit={fetch}
                        className="row col-md-12 border m-2 p-3"
                        style={{ borderRadius: '10px' }}
                    >
                        <div className="input-group mb-3 col-md-4">
                            <label className="form-label mt-1 mr-2">Modified Date From</label>
                            <input
                                type="date"
                        ...
                        <div className="input-group mb-3 col-md-4">
                            <label className="form-label mt-1 mr-2">Modified Date To</label>
                            <input
                                type="date"
                                className="form-control"
                                id="to_date"
