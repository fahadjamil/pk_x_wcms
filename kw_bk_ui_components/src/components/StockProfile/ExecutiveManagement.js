...
export const ExecutiveManagement = (props) => {
    const { lang } = props;
    ...
    useEffect(() => {
        if (stockID) {
            Axios.get(marketBackEndProxyPass(), {
                ...
                .then((res) => {
                    if (res && res.data && res.data['Executive Management']) {
                        setTableData(res.data['Executive Management']);
                    } else {
    ...
    const TITLE = convertByLang('الإدارة التنفيذية', 'Executive Management');
    ...
    const ExecutiveManagementComponent = () => (
        <div>
            <h3>{TITLE}</h3>
    ...
    return dataUnavailable ? <DataUnavailable lang={lang} /> : <ExecutiveManagementComponent />;
};
