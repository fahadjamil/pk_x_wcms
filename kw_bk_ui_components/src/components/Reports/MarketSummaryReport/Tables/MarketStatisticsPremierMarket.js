...
export const MarketStatisticsPremierMarket = (props) => {
    const { lang } = props;
    ...
    useEffect(() => {
        Axios.get(marketBackEndProxyPass(), {
            params: { RT: 3567, Y: year, M: month, MT: 'P', T: reportMode },
        }).then((res) => {
            if (res && res.data) {
    ...
    return (
        <div>
            <TableHeadingWrapper>
                {convertByLang('أداء السوق الأول', 'Premier Market Performance')}
            </TableHeadingWrapper>
